...
const Main: FC = () => {
  ...
  useEffect(() => { setCookie('open_id', searchParams.get('open_id'), 1) }, [])
  ...
  */
  ...
  useEffect(() => {
    if (APP_INFO?.title)
      document.title = `${APP_INFO.title} - ChatBank`
  }, [APP_INFO?.title])
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const prompt_variables = userInputsFormToPromptVariables(user_input_form, paramsObj)
        setPromptConfig({
